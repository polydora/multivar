...
p_load <- ggplot(df_load) + 
  geom_text(aes(x = PC1, y = PC2, label = rownames(df_load)), 
            size = 6, vjust = df_load$vjust, hjust = df_load$hjust) + 
...
p_scores <- ggplot(df_scores, aes(x = PC1, y = PC2, colour = region)) + 
  geom_text(aes(label = country), size = 6) +
...
- Линия регрессии --- направление минимальноо разброса значений зависимой переменной (сумма квадратов остатков).
