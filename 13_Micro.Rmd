...
######setwd("C:/data/")
###не забывать менять рабочую директорию между проектами
#я в итоге стала писать полные пути до файлов потому что у меня на винде не работает ничего
...
feature_mat <- read.delim("data/master_feature_new.txt", stringsAsFactors = FALSE)
tax_mat <- read.delim("data/master_taxonomy_new.txt", stringsAsFactors = FALSE)
samples_df <- read.delim("data/master_meta_new.txt", stringsAsFactors = FALSE)
...
#посмотрим на самые низкие глубины прочтения
head(sort(sample_sums(snails_filtred), decreasing = FALSE, na.last = TRUE), 15)

# А как посмотреть на 10 образцов с самым большим числом ридов?



#варьирование на порядок можно пережить, на 2 - нет
...
```{r}
tail(sort(sample_sums(snails_subsampled_2), decreasing = FALSE, na.last = TRUE), 15) 
```
...
#А как визуализировать глубину прочтений до нормализации?
...
## Выровненность и Шеннон общие руками
...
#попробуйте повторить ординацию для этого вида, но на другой матрице расстояний или другим методом ординации
