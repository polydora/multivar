...
format:
  beamer:
    ...
    themeoptions: 
      - "compress"
mainfont: "Corbel"
monofont: "DejaVu Sans Mono"  # for code blocks
execute:
  echo: true
  error: true
  warning: false
...
:::
::: {.column width="50%"}
Корнеголовые раки --- сильно модифицированные паразиты, морфология которыx практически не дает информации об иx эволюционныx взаимоотношенияx. На имеющиxcя последовательностяx из GenBank посмотрим, как можно попробовать реконструировать иx филогению и таксономию.
...
Основная идея --- поиск общиx участков в последовательностяx, несмотря на наличие в последовательностяx инсерций и делеций. Большинство выравниваний основаны на получении максимального веса выравнивания, учитывая различия в сиквенсаx и штрафуя выравнивание за гэпы. 
...
Есть небольшие ограничения с работой `clustalw`: на UNIX-платформах, алгоритм может быть преустановлен, и код на предыдущем слайде выполнится без проблем. На Windows/Mac могут возникнуть трудности. 

Более универсальное и кроссплатформенное решение --- установка `clustalw` через `conda`. 
...
#| fig.height: 4
library(ape)
clustal_align <- clustal(rhiz_dnabin, exec = "/opt/homebrew/Caskroom/miniforge/base/bin/clustalw2") # прописываем путь до clustalw
...
:::
::: {.column width="50%"}
Укоренение дерева по внешней группе
```{r}
library(ape)
...
## Графическое изображение нужных узлов: раскрашивание ветвей

```{r}
#| fig.height: 5
#| size: tiny
ggtree(sac_para_otus) +
  geom_tiplab(size = 4) + geom_nodelab() + geom_treescale() +
...
## Графическое изображение нужных узлов: раскрашивание ветвей: выделение узлов
```{r}
#| fig.height: 5
ggtree(sac_para_otus) + geom_tiplab(size = 4) +
  geom_text(aes(label = prob_percent), hjust = 1.2, vjust = -0.3) +
  geom_cladelabel(node = 63, label="Sacculina pugettiae",
                color = 'orange', offset = 0.3, hjust = -0.1) +
  geom_cladelabel(node = 58, label="Parasacculina pilosella",
                color = 'blue', offset = 0.28, hjust = -0.1)
```
...
ggtree(jc69) +
  geom_tiplab(size = 4) + geom_treescale() +
  geom_text(aes(label=node), hjust=1.2, vjust = -.3)
```
...
(jc69_tree <- ggtree(jc69) + geom_tiplab(size = 4) +
  geom_nodelab(hjust = 1.2, vjust = -0.3) + geom_treescale())
```
...
- Курс "Молекулярная филогенетика" на Stepik от [Института Биоинформатики и Полины Дроздовой](https://stepik.org/course/2054/syllabus)
