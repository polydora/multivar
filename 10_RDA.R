...
# В основе RDA лежит математическое понятие редундантности (избыточности) — предсказуемости одной части данных на основе другой.

# Индекс избыточности (Redundancy Index) — это ключевая метрика в RDA. Он показывает, какая доля дисперсии в наборе зависимых переменных (Y) объясняется (или может быть предсказана) набором независимых переменных (X). Если индекс высок, это означает высокую избыточность: Y не несет новой информации сверх того, что есть в X.
