...
Исходная матрица имеет размерность `r dim(protein[, -c(1,2)])`

Сколько исходных осей?
Сколько объектов?

## PCA данных по протеинам

```{r}
prot_pca <- rda(protein[, -c(1,2)], scale = FALSE)
```

## Ординация в новых осях
...
Как определить долю дисперсии, которую описывает каждая из новых осей?

## SVD исходной матрицы

```{r}
prot_pca <- rda(protein[, -c(1,2)], scale = FALSE)
prot_svd <- svd(scale(protein[, -c(1,2)], center = TRUE, scale = FALSE))
...
```{r}
plot(x = prot_pca$CA$eig, y = D)

```

- Всего признаков (изначальных осей): `r ncol(protein[, -c(1,2)])`
- Квадратичная зависимость: собственное число из PCA = D^2^ из SVD

## Собственные векторы (базис новой системы координат) и левая сингулярная матрица **U**

```{r}
plot(x = prot_pca$CA$u, y = U)
```

Это одно и тоже!


## Координаты объектов в осях главных компонент и правая сингулярная матрица **V**

```{r}
plot(scores(prot_pca, choices = 1:9)$sites, V)
```
...
Для ординации объектов в новой системе координат, заданной главными компонентами, нет необходимости проводить спектральное разложение: 

X -> A -> $\lambda$, $U$ -> Новые координаты  

Можно короче

X -> SVD -> $U, D, V$ -> Новые координаты
...
library(vegan)
bird_pca <- rda(birds[ , -c(1, 2)], scale = FALSE)
# summary(bird_pca)
