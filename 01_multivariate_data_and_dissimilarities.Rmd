...
pl4 <- 
  ...
  geom_point(size = 5, color = "red") + xlim(0,5) + 
  ylim(0, 11) + 
  annotate(geom = "segment", x = 0, y = 0, xend = 2, yend = 5, arrow = arrow(type = "closed", angle = 20, ends = "last"), size = 1) + 
  annotate(geom = "segment", x = 0, y = 0, xend = 2, yend = 10, arrow = arrow(type = "closed", angle = 20, ends = "last"), size = 1) + 
  geom_text_repel(aes(label = object)) + 
  annotate(geom = "text", x = 1, y = 2, label = "LV 1", angle = 0) + 
  annotate(geom = "text", x = 1, y = 6, label = "LV 2", angle = 0) + 
  annotate(geom = "text", x = 1, y = 3.3, parse = TRUE, label = "alpha", size = 10, angle = 0)

pl5 <- 
  ggplot(dat2, aes(x = x, y = y)) + 
  geom_point(size = 5, color = "red") + xlim(0,5) + 
  ylim(0, 11) + 
  geom_segment(aes(x = 0, y = 0, xend = x, yend = y), arrow = arrow(type = "closed", angle = 20, ends = "last"), size = 1) + 
  geom_segment(aes(x = 0, y = 0, xend = 2, yend = 10), arrow = arrow(type = "closed", angle = 20, ends = "last"), size = 1) + 
  geom_text_repel(aes(x = x, y = y+0.5), label = dat2$object)
...
pl6 + 
  geom_text_repel(aes(x = x, y = y+0.5), label = dat2$object,  box.padding = 0.6, min.segment.length = 0.5) 
...
## Облако точек в n-мерном пространстве
...
set.seed(123)
n_groups <- 4  # количество групп верхнего уровня
n_subgroups <- 3  # количество подгрупп в каждой группе
n_points <- 130  # точек в каждой подгруппе
...
data <- expand.grid(
  ...
  mutate(
    ...
    # Смещения для подгрупп
    subgroup_x = case_when(
      subgroup == 1 ~ -2,
      subgroup == 2 ~ 0,
      subgroup == 3 ~ 2
    ),
    subgroup_y = case_when(
      subgroup == 1 ~ 1,
      subgroup == 2 ~ -1,
      subgroup == 3 ~ 0
    ),
    # Финальные координаты с случайным разбросом
    x = group_x + subgroup_x + rnorm(n(), 0, 0.5),
    y = group_y + subgroup_y + rnorm(n(), 0, 0.5),
    group_subgroup = paste(group, subgroup, sep = "-")
  )

# Визуализация
ggplot(data, aes(x = x, y = y)) +
  # Точки с цветом по группам и формой по подгруппам
  geom_point(size = 2, alpha = 0.7) +
  labs(x = "Признак 1", y = "Признак 2") +
  theme_minimal()

```

Что может лежать в основе такого паттерна расположения точек?
...
Pl_point <- 
  ...
  geom_segment(aes(x = X, y = Y, xend = c(0, 2), yend = c(1, 0)), linetype = 2) + geom_segment(aes(x = X, y = Y, xend = c(1, 0), yend = c(0, 2)), linetype = 2) + 
  xlim(0, 3) + 
  ylim(0,3) + 
  geom_point(size = 4, color = "red") +  
  geom_text_repel(aes(label = c("Object 1", "Object 2"))) + 
  labs(x = "Признак A", y = "Признак В")
...
pl7 + 
  geom_text_repel(data = dat2, aes(x = x, y = y, label = object)) + 
  labs(x = "Признак A", y = "Признак В")
...
```{r}
Pl_rel + 
  annotate(geom = "text", x = 0.55, y = 220, label = "Внутригрупповые") + 
  annotate(geom = "text", x = 0.9, y = 120, label = "Межгрупповые") + 
  labs(x = "Мера расстояния", y = "Частота", title = "")
```
...
```{r}
ggplot(dists, aes(x = Euclidean, y = Bray)) + 
  geom_point() + 
  theme_bw()
```
...
```{r}
ggplot(abund_log, aes(x = Scoloplos_armiger, y = Terebellides_stroemi)) + 
  geom_point(color = "red", size = 4) + 
  labs(x = "Вид 1", y = "Вид 2")
```
...
ggplot(New_coord, aes(x=Coord, y =0)) + 
  geom_segment(x = 0, xend = d, y = 0, yend=0, size = 2, color = "red") + 
  geom_point(fill = "yellow", position = position_jitter(width = 0, height=0), size = 4, shape = 21) + 
  ylim(-1, 1) 
```
...
```{r}
ggplot(New_coord, aes(x=Coord, y =0)) + 
  geom_segment(x = 0, xend = d, y = 0, yend=0, size = 1, color = "black") +
  geom_point(aes(fill = hydrol$Temp), position = position_jitter(width = 0, height=0.1), size = 4, shape = 21) + 
  ylim(-1, 1) + 
  scale_fill_gradient(low = "yellow", high = "red") + 
  xlab("Координаты на полярной оси №1") + ylab("")  + 
  labs(fill = "Температура придонной воды") + 
