...
library(jpeg)
fisher <- readJPEG("images/fisher.jpg")
...
fisher_XY <- melt(fisher) ## Переводим матрицу в два вектора координат и вектор значений интенсивности заливки

names(fisher_XY) <- c("X1", "X2", "value")

ggplot(fisher_XY, aes(X1, X2)) + geom_point(aes(color = value), size = 5) + scale_fill_gradient(low = "darkblue",   high =  "white" )
